...
class UNet_UAPSv2(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_UAPSv2, self).__init__()
    ...
    def forward(self, x):
        ...
        out1 = self.diaconv1(out)
        out2 = self.diaconv2(out)
        out3 = self.diaconv3(out)
        out = out1 + out2 + out3
        att_map = self.att(out)



        return main_seg, aux_seg1, aux_seg2, aux_seg3, att_map
...
class refine(nn.Module):
    def __init__(self, class_num):
        ...
        self.out_conv = nn.Conv2d(16, self.n_class, kernel_size=3, padding=1)

    def forward(self, att1, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = x * att1
        x = self.deconv(x)
